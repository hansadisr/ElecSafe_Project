"""
Vehicle Detection Server for Raspberry Pi (Daytime Model)
Run this on port 5001

Install: pip install flask flask-cors opencv-python ultralytics requests gpiozero
...
def detection_loop():
    ...
    print("Starting vehicle detection loop (Daytime Model)...")
    
    # Load YOLO model - Daytime model
    ...
    while True:
        ...
        out = cv2.VideoWriter('/home/pi/yolo/output_daytime_vehicle.mp4', fourcc, 20.0,
                              (int(cap.get(3)), int(cap.get(4))))
        
        print("Vehicle detection started (Daytime mode)")
        ...
        while True:
            ...
            with detection_lock:
                ...
                if detection_confirmed >= FRAMES_TO_CONFIRM and not elephant_present:
                    ...
                    send_telegram_alert("ALERT: Elephant entered the frame! [Vehicle Detection]")
                ...
                # Elephant left
                    ...
                    send_telegram_alert("Elephant left the frame. [Vehicle Detection]")
...
def get_status():
    """Returns current vehicle detection status"""
    with detection_lock:
        if elephant_detected:
            return jsonify({"status": "detected", "type": "vehicle"}), 200
        else:
            return jsonify({"status": "clear", "type": "vehicle"}), 200

@app.route('/health', methods=['GET'])
def health_check():
    return jsonify({"message": "Vehicle detection server is running"}), 200
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    print("Server will run on http://10.182.66.142:5001")
    print("Dashboard URL: http://localhost:3000/vehicle")
